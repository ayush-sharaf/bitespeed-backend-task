...
export class ContactService {
    ...
    async findAllLinkedContacts(contactIds) {
    ...
    async createContact(email, phoneNumber, linkedId, linkPrecedence = "primary") {
      ...
      VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING *`, [phoneNumber || null, email || null, linkedId || null, linkPrecedence]);
        return result[0];
    }
    async updateContactToSecondary(contactId, linkedId) {
        const result = await sql.query(`UPDATE "Contact" SET "linkedId" = $1, "linkPrecedence" = 'secondary', "updatedAt" = NOW() WHERE id = $2 RETURNING *`, [linkedId, contactId]);
        return result[0];
    }
    async updateAllSecondaryContacts(oldPrimaryId, newPrimaryId) {
        await sql.query(`UPDATE "Contact" SET "linkedId" = $1, "updatedAt" = NOW() WHERE "linkedId" = $2`, [newPrimaryId, oldPrimaryId]);
    }
    consolidateContacts(contacts) {
        ...
        const emails = Array.from(new Set(contacts.map(c => c.email).filter((e) => e !== null)));
        const phoneNumbers = Array.from(new Set(contacts.map(c => c.phoneNumber).filter((p) => p !== null)));
        return {
