...
export const identifyController = async (req, res) => {
    ...
    const hasNewInfo = (email && !allLinked.some(c => c.email === email)) ||
    ...
    if (primaries.length > 1) {
        const [truePrimary, ...toUpdate] = primaries.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
